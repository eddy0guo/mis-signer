...
class utils {
    ...
    async get_receipt(txid) {
        const cmd = 'curl -X POST --data \'\{\"id\":1, \"jsonrpc\":\"2.0\",\"method\":\"asimov_getTransactionReceipt\",\"params\":\[\"' + txid + '\"\]\}\}\' -H \"Content-type: application\/json\" ' + cfg_1.default.asimov_child_rpc;
    ...
    async orderTobytes(order) {
        ...
        let encode = ethabi.rawEncode(['bytes32', 'address', 'address', 'address', 'address', 'address', 'uint256', 'uint256', 'bytes32'], ['0x45eab75b1706cbb42c832fc66a1bcdaafebcdaea71ed2f08efbf3057c588fcb6',
    ...
    judge_legal_num(num) {
        ...
        else if (number_precision_1.default.times(num, 10000) != Math.floor(number_precision_1.default.times(num, 10000))) {
            console.error('cannt support this decimal', num, number_precision_1.default.times(num, 10000), num * 10000);
            result = false;
        }
        return result;
    }
    async decode_transfer_info(txid) {
        const cmd = 'curl -X POST --data \'\{\"id\":1, \"jsonrpc\":\"2.0\",\"method\":\"asimov_getRawTransaction\",\"params\":\[\"' + txid + '\",true,true\]\}\}\' -H \"Content-type: application\/json\" ' + cfg_1.default.asimov_chain_rpc;
        const sto = child.execSync(cmd);
        ...
        const transfer_info = {
            ...
            vin_amount: number_precision_1.default.divide(vin_amount, 100000000),
            to_amount: number_precision_1.default.divide(vout_to_amount, 100000000),
            remain_amount: number_precision_1.default.divide(vout_remain_amount, 100000000),
            fee_amount: number_precision_1.default.divide(txinfo.fee[0].value, 100000000),
    ...
    async decode_erc20_transfer(txid) {
        ...
        const info = {
            ...
            amount: number_precision_1.default.divide(amount, 100000000),
        };
        return info;
    }
}
