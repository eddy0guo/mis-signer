...
event orderhashmsg(bytes32 bs);
  ...
  function hashordermsg2(Order _order) public returns(bytes32){
       Order[] memory ord=new Order[](1);
       ord[0]=_order;
        return hashmsg(getorderhash(ord))[0];
    }
     ...
     function getorderhash(Order[] _order)   public  returns(bytes32[]){
         bytes32[]  memory orderbs=new bytes32[](_order.length);
         for(uint256 i=0;i<_order.length;i++){
            orderbs[i]=keccak256(abi.encodePacked(
                EIP712_ORDERTYPE,
                bytes32(_order[i].taker),
                bytes32(_order[i].maker),
                bytes32(_order[i].baseToken),
                bytes32(_order[i].quoteToken),
                bytes32(_order[i].relayer),
                _order[i].baseTokenAmount,
                _order[i].quoteTokenAmount,
                bytes32(keccak256(_order[i].takerSide))
                ));
         }
         return orderbs;
    ...
    bytes32 bts;
     ...
     function  hashmsg( bytes32[] _hashmsg) public  returns(bytes32[]) {
         bytes32[] memory orderbs=new bytes32[](_hashmsg.length);
         for(uint256 i=0;i<_hashmsg.length;i++){
             orderbs[i]=keccak256(abi.encodePacked("\x19\x01",EXCHANGE_SEPARA,_hashmsg[i]));
             emit orderhashmsg(orderbs[i]);
         }
         return orderbs;
	...
	//takerside为taker吃掉的单子方向，吃掉买单side为buy，反之sell
        Order memory order=getorders(TradeParams[i],orderAddressSet);
                    bytes32 bs= hashordermsg2(order);
