...
export default () => {
    ...
    wallet.all('/faucet/:address', async (req, res) => {
        ...
        for (const i in token_arr as any[]) {
            setTimeout(async () => {
                const [err, result] = await to(wallet.commonTX.transfer(address, faucet_amount[i], token_arr[i].asim_assetid));
                results.push[result];
    ...
    wallet.all('/faucet_asim/:address', async (req, res) => {
        ...
        const [err, result] = await to(wallet.commonTX.transfer(address, 10000, token_info[0].asim_assetid));
    ...
    wallet.all('/faucet3/:address', async (req, res) => {
        ...
        for (const i in token_arr as any[]) {
            setTimeout(async () => {
                const [err, result] = await to(wallet.commonTX.transfer(address, big_faucet_amount[i], token_arr[i].asim_assetid));
                results.push[result];
                if (err) console.log(err);
            }, Number(i) * 20000);
        }
    ...
    wallet.all('/my_converts2/:address/:page/:perpage', async (req, res) => {
        ...
        const [err, result] = await to(psql_db.my_converts2([address, offset, perpage]));
        res.json({ result, err });
    });
    ...
    wallet.all('/sendrawtransaction/asset2coin_v2/:sign_data', async (req, res) => {
        const sign_data = [req.params.sign_data];
        const [master_err, master_txid] = await to(chain.sendrawtransaction(sign_data));
        ...
        if (master_err == undefined) {
            ...
            setTimeout(async () => {
                const [decode_err, decode_info] = await to(utils.decode_transfer_info(master_txid));
    ...
    wallet.all('/sendrawtransaction/coin2asset_v2', async (req, res) => {
        ...
        const info = ['MIST_BURN', tokens[0].address, mist_config.bridge_address, amount, expire_time];
        const str = info.join('');
        ...
        const [master_err, master_txid] = await to(master_wallet.commonTX.transfer(address, amount, tokens[0].asim_assetid));
    ...
    wallet.all('/sendrawtransaction/asset2coin_v3/:sign_data', async (req, res) => {
        ...
        if (master_err == undefined) {
            ...
            setTimeout(async () => {
                ...
                const [err3, result3] = await to(psql_db.update_asset2coin_decode(update_info_arr));
    ...
    wallet.all('/burn_coin_tohex/:address/:token_name/:amount', async (req, res) => {
        ...
        const info = ['MIST_BURN', tokens[0].address, mist_config.bridge_address, amount, expire_at];
    ...
    wallet.all('/my_converts_v3/:address/:token_name/:page/:perpage', async (req, res) => {
        ...
        const offset = (+page - 1) * +perpage;
        const [err, result] = await to(psql_db.my_bridge_v3([address, token_name, offset, perpage]));
        const success = result == undefined ? false : true;
        res.json({ success, result, err });
    });
    ...
    wallet.all('/get_blockchain_info', async (req, res) => {
        ...
        const [err2, result2] = await to(chain.getblockchaininfo(undefined, 'child_poa'));
