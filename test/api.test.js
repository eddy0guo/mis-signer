...
describe('API', function() {
	...
	it('mist_user_overview', async() =>{
		const start = new Date().getTime()
		let result = await rp("http://127.0.0.1:21000/adex/mist_user_overview/0x66a6c15b2a6a253aa2f67ea525b2619f12ae8d3509")
		const end = new Date().getTime()
		result = JSON.parse(result);
		expect(result.success).to.be.equal(true);
	...
	it('asset_balances', async() =>{
		const start = new Date().getTime()
		let result = await rp("http://127.0.0.1:21000/adex/asset_balances/0x66ea4b7f7ad33b0cc7ef94bef71bc302789b815c46")
		const end = new Date().getTime()
		result = JSON.parse(result);
		expect(result.success).to.be.equal(true);
		expect(end-start).to.be.lessThan(10000)
	...
	it('erc20_balances', async() =>{
		const start = new Date().getTime()
		let result = await rp("http://127.0.0.1:21000/adex/erc20_balances/0x66ea4b7f7ad33b0cc7ef94bef71bc302789b815c46")
		const end = new Date().getTime()
		result = JSON.parse(result);
		expect(result.success).to.be.equal(true);
		expect(end-start).to.be.lessThan(10000)
...
/*
	it('build_order', async() =>{
		...
		var options = {
			method: 'POST',
			uri: 'http://127.0.0.1:21000/adex/build_order_v3',
			body: {
			signature:{
	...
	it('cancle_orders', async() =>{
		const start = new Date().getTime()
		let result = await rp("http://127.0.0.1:21000/adex/")
		const end = new Date().getTime()
		result = JSON.parse(result);
		expect(result.success).to.be.equal(true);
		expect(end-start).to.be.lessThan(300)
...
*/
	it('burn_coin_tohex', async() =>{
		const start = new Date().getTime()
		let result = await rp("http://127.0.0.1:21000/wallet/burn_coin_tohex/0x66d130d64c8d988800664616383a2bda48ffd4eecc/CNYC/1")
		const end = new Date().getTime()
		result = JSON.parse(result);
		expect(result.success).to.be.equal(true);
		expect(end-start).to.be.lessThan(2000)
	...
	it('find_convert', async() =>{
		const start = new Date().getTime()
		let result = await rp("http://127.0.0.1:21000/wallet/find_convert/8c4ddabebe95718a37aea074120d3bd133196c01812935ddef42dffcdfd431ac")
		const end = new Date().getTime()
		result = JSON.parse(result);
		expect(result.success).to.be.equal(true);
		expect(end-start).to.be.lessThan(300)
	...
	it('coin2asset_fee_config', async() =>{
		const start = new Date().getTime()
		let result = await rp("http://127.0.0.1:21000/wallet/coin2asset_fee_config")
		const end = new Date().getTime()
		result = JSON.parse(result);
		expect(result.success).to.be.equal(true);
		expect(end-start).to.be.lessThan(300)
	...
	it('list_fingo_config', async() =>{
		const start = new Date().getTime()
		let result = await rp("http://127.0.0.1:21000/wallet/list_fingo_config")
		const end = new Date().getTime()
		result = JSON.parse(result);
		expect(result.success).to.be.equal(true);
		expect(end-start).to.be.lessThan(300)
